...
//task 1
type GenreArray = string[]

function createNewGenreArray(films: Film[]): GenreArray {
    return [
        ...new Set(films.map(({ genre }) => genre).flat())
    ]
}

// task 2

type ActorsArray = string[]

function createNewActorsArray(films: Film[]): ActorsArray {
    return [
        ...new Set(films.map(({actors}) => actors).flat())
    ]
}

//task 3

function sortFilmsByRating(films: Film[]): Film[] {
...
function createNewFilmsArray (films: Film[]): NewFilmObject[] {
...
console.log(createNewFilmsArray(films));

// task 5

function filterFilmsByYear(films: Film[], date: number): Film[] {
    return films.filter(({year}) => year === date)
}

// task 6

function filterFilmsByTitle(films: Film[], name: string): Film[] {
    return films.filter(({title}) => title === name)
}

//task 7
function filterFilmsByTitleAndPlot(films: Film[], string: string): Film[] {
    return films.filter(({ title, plot }) =>
        title.toLowerCase() === string.toLowerCase() || plot.toLowerCase().includes(string.toLowerCase()))
}
